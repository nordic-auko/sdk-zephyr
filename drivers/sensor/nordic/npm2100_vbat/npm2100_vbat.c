...
static const struct linear_range oversample_range = LINEAR_RANGE_INIT(1, 1, 0U, 4U);
			     ...
			     struct sensor_value *valp)
{
	...
	int32_t tmp = ((int32_t)*result * VBAT_SCALING_MUL) / VBAT_SCALING_DIV;
...
int npm2100_vbat_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	for (int idx = 0; idx < ARRAY_SIZE(data->adc); idx++) {
		...
		const uint8_t oversampling =
			BIT(FIELD_GET(data->adc[idx].config, ADC_CONFIG_AVG_MASK));
		int32_t delay_usec;
				 ...
				 enum sensor_attribute attr, struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_OVERSAMPLING) {
		...
		*val = (struct sensor_value){
			.val1 = FIELD_GET(adc_cfg->config, ADC_CONFIG_AVG_MASK), .val2 = 0};
	...
	for (int idx = 0; idx < ARRAY_SIZE(npm2100_attr); idx++) {
		if ((npm2100_attr[idx].chan == chan) && (npm2100_attr[idx].attr == attr)) {
			...
			reg_data = FIELD_GET(reg_data, npm2100_attr[idx].reg_mask);
				 ...
				 enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((chan == SENSOR_CHAN_GAUGE_VOLTAGE) &&
		...
		ret = linear_range_get_index(&adcdelay_range, sensor_value_to_micro(val), &delay);
		if (ret == -EINVAL) {
			return ret;
		}
		...
		if (delay == 0) {
			adc_cfg->config |=
				FIELD_PREP(ADC_CONFIG_MODE_INS_VBAT, ADC_CONFIG_MODE_MASK);
		} else {
			adc_cfg->config |=
				FIELD_PREP(ADC_CONFIG_MODE_DEL_VBAT, ADC_CONFIG_MODE_MASK);
		}
	...
	if (attr == SENSOR_ATTR_OVERSAMPLING) {
		...
		ret = linear_range_get_index(&oversample_range, sensor_value_to_micro(val),
					     &oversample);
		if (ret == -EINVAL) {
			return ret;
		}
		...
		adc_cfg->config |= FIELD_PREP(oversample, ADC_CONFIG_AVG_MASK);
